...
namespace GraphicsProject.Helpers
{
        ...
        where T : struct
    {
        ...
        public VBO(T[] Data, int Length, BufferTarget Target = BufferTarget.ArrayBuffer, BufferUsageHint Hint = BufferUsageHint.StaticRead)
        {
            Length = Math.Max(0, Math.Min(Length, Data.Length));

            this.vboID = GLMethods.CreateVBO<T>(this.BufferTarget = Target, Data, Hint, Length);
        ...
        /// <param name="Target">Specifies the target buffer object.</param>
        /// <param name="Hint">Specifies the expected usage of the data store.</param>
        public VBO(T[] Data, BufferTarget Target = BufferTarget.ArrayBuffer, BufferUsageHint Hint = BufferUsageHint.StaticRead)
        {
            this.vboID = GLMethods.CreateVBO<T>(this.BufferTarget = Target, Data, Hint);
        ...
        ~VBO()
        {
            if (this.vboID != 0) System.Diagnostics.Debug.Fail("VBO was not disposed of properly.");
        }
        #endregion
